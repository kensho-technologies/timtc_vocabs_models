...
class MPTModel(MPTPreTrainedModel):
    ...
    @torch.no_grad()
    def _attn_bias(self, device: torch.device, dtype: torch.dtype, attention_mask: Optional[torch.ByteTensor]=None, prefix_mask: Optional[torch.ByteTensor]=None, sequence_id: Optional[torch.LongTensor]=None):
...
class MPTForCausalLM(MPTPreTrainedModel):
    ...
    def set_output_embeddings(self, new_embeddings: Union[SharedEmbedding, nn.Embedding]):
    ...
    def prepare_inputs_for_generation(self, input_ids: torch.Tensor, past_key_values: Optional[List[Tuple[torch.Tensor, torch.Tensor]]]=None, inputs_embeds: Optional[torch.Tensor]=None, **kwargs: Any):
        if inputs_embeds is not None:
    ...
    @staticmethod
    def _reorder_cache(past_key_values: List[Tuple[torch.Tensor, torch.Tensor]], beam_idx: torch.LongTensor):
